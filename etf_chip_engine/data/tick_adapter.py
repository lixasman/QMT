...
def ticks_to_snapshots(ticks: Any) -> pd.DataFrame:
    ...
    if isinstance(ticks, pd.DataFrame):
        ...
        if tcol is None:
            ...
            if isinstance(tcol, pd.DatetimeIndex) or (hasattr(tcol, "dtype") and np.issubdtype(getattr(tcol, "dtype", object), np.datetime64)):
        ...
        high_raw = pd.to_numeric(ticks["high"], errors="coerce").fillna(0.0).to_numpy(dtype=np.float64) if "high" in cols else close.copy()
        low_raw = pd.to_numeric(ticks["low"], errors="coerce").fillna(0.0).to_numpy(dtype=np.float64) if "low" in cols else close.copy()
        amount_cum = pd.to_numeric(ticks["amount"], errors="coerce").fillna(0.0).to_numpy(dtype=np.float64) if "amount" in cols else np.zeros(len(ticks), dtype=np.float64)
        volume_cum = pd.to_numeric(ticks["volume"], errors="coerce").fillna(0.0).to_numpy(dtype=np.float64) if "volume" in cols else np.zeros(len(ticks), dtype=np.float64)
        iopv = pd.to_numeric(ticks["iopv"], errors="coerce").fillna(0.0).to_numpy(dtype=np.float64) if has_iopv else None
        ...
        if open_.size:
            open_[0] = close[0]
        amount_delta = np.diff(amount_cum, prepend=amount_cum[:1])
        volume_delta = np.diff(volume_cum, prepend=volume_cum[:1])
        if amount_delta.size:
            amount_delta[0] = 0.0
        ...
        if has_iopv and iopv is not None:
            prem = np.zeros(len(ticks), dtype=np.float64)
            valid = (iopv > 0) & np.isfinite(iopv) & np.isfinite(close)
            if valid.any():
                prem[valid] = (close[valid] - iopv[valid]) / iopv[valid]
            out["iopv"] = iopv
            out["premium_rate"] = prem
    ...
    has_iopv = "iopv" in names
    iopv = _get("iopv") if has_iopv else None
    open_ = np.roll(close, 1)
    if open_.size:
        open_[0] = close[0]
    high_raw = _get("high")
    ...
    if has_iopv and iopv is not None:
        prem = np.zeros(arr.shape[0], dtype=np.float64)
        valid = (iopv > 0) & np.isfinite(iopv) & np.isfinite(close)
        if valid.any():
            prem[valid] = (close[valid] - iopv[valid]) / iopv[valid]
        out["iopv"] = iopv
